...
module.exports = {
  addLocalisation: async (req, res) => {
    ...
    try {
      ...
      return res
        .status(201)
        .json({ error: false, message: "Succes: Location have been added" });
    } catch (err) {
      return res
        .status(500)
  ...
  deleteLocalisation: async (req, res) => {
    try {
      ...
      return res
        .status(201)
        .json({ error: false, message: "Succes: Location have been deleted" });
    } catch (err) {
      return res
        .status(500)
        .json({ error: true, message: "Internal Servor Error" });
    ...
    try {
      ...
      return res
        ...
        .json({
          error: false,
          loc,
          message: "Succes: Locations have been Geted",
        });
    } catch (err) {
      return res
        .status(500)
